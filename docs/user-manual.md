...
13. [이미지 내보내기](#이미지-내보내기)
14. [파일 관리](#파일-관리)
15. [키보드 단축키 전체 목록](#키보드-단축키-전체-목록)
16. [경쟁 제품 대비 장점](#경쟁-제품-대비-장점)
17. [FAQ](#faq)
...
MindForge는 macOS/Windows/Linux에서 동작하는 네이티브 데스크톱 마인드맵 애플리케이션입니다. XMind 파일 형식(.xmind)과 완벽하게 호환되며, 빠른 Canvas 렌더링과 직관적인 UI를 제공합니다.
...
각 항목마다 12가지 프리셋 색상과 직접 입력(HEX) 기능을 제공합니다.
...
| **Logic Chart** | 왼쪽에서 오른쪽으로 분기 | 논리적 흐름, 의사결정 트리 |
...
마인드맵을 이미지 파일로 내보내어 문서, 프레젠테이션, 메신저 등에서 활용할 수 있습니다.

### 사용 방법

1. 툴바에서 **🖼 Export** 버튼을 클릭합니다
2. 드롭다운 메뉴에서 형식을 선택합니다:
   - **PNG 내보내기** — 래스터 이미지 (고해상도 2x)
   - **SVG 내보내기** — 벡터 이미지 (무한 확대 가능)
...
| 항목 | PNG | SVG |
|------|-----|-----|
| 용도 | 문서 삽입, 메신저 공유, SNS | 프레젠테이션, 인쇄, 웹 |
| 확대 | 2x 해상도 (Retina 대응) | 무한 확대 가능 |
| 편집 | 불가 | 일러스트레이터 등에서 편집 가능 |
| 파일 크기 | 중간 | 작음 |
| 배경 | 테마 배경색 포함 | 테마 배경색 포함 |

### 내보내기 특징

- 전체 맵이 자동으로 포함됩니다 (줌/팬 상태와 무관)
- 적절한 여백(padding)이 자동 추가됩니다
- 파일명은 현재 파일명을 기반으로 자동 생성됩니다 (예: `mymap.png`)
- 접힌 토픽의 하위 항목은 내보내기에 포함되지 않습니다
...
**MindForge의 핵심 차별점**: XMind 파일을 그대로 열고 저장할 수 있으면서 무료이고, 네이티브 앱이라 훨씬 빠릅니다.

### vs Miro / MindMeister (웹 기반)

| 항목 | MindForge | Miro / MindMeister |
|------|-----------|---------------------|
...
**MindForge의 핵심 차별점**: 인터넷 없이 동작하고, 데이터가 로컬에만 저장되므로 개인정보 보호에 강합니다. 네이티브 Canvas 렌더링으로 수백 개 노드에서도 부드럽습니다.

### vs FreeMind / Freeplane (오픈소스)

| 항목 | MindForge | FreeMind / Freeplane |
|------|-----------|----------------------|
...
| **Zen Mode** | 지원 | 미지원 |
| **아웃라이너** | 원클릭 전환 | 미지원 |
| **이미지 내보내기** | PNG/SVG | PNG만 |
| **앱 크기** | ~15MB | ~50MB (JRE 포함) |

**MindForge의 핵심 차별점**: 최신 기술 스택으로 만들어진 현대적 UI, 가볍고 빠른 실행, 업계 표준 XMind 형식 지원.

### MindForge만의 고유 장점 요약
...
A: 데이터 모델은 다중 시트를 지원하며, UI는 향후 업데이트에서 지원 예정입니다.
...
*MindForge v1.0.0 | 마지막 업데이트: 2026-03-19*
